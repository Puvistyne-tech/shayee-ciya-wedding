...
export default function SaveTheDate() {
  ...
  const calendarStrings: CalendarStrings = {
    ...
    ceremonySummary: tCalendar("ceremonySummary"),
    receptionSummary: tCalendar("receptionSummary"),
    filename: tCalendar("filename"),
  };
  ...
  const handleAddToCalendar = () => {
    ...
    if (isMobile) {
      window.location.href = getCalendarApiUrl(locale);
      return;
    }

    downloadICS(calendarStrings);
  };
  ...
  return (
    <section className="border-y border-stone-200 bg-gradient-to-b from-white to-cream px-6 py-14 text-center">
      ...
      <p className="mt-1 text-sm text-stone-600">{t("times")}</p>
      <p className="mt-2 text-sm text-stone-500">{getWeddingLocation()}</p>
      ...
      <p className="mt-3 text-xs text-stone-500">{t("helper")}</p>

      <div className="mx-auto mt-6 flex max-w-sm flex-col gap-2 sm:flex-row sm:justify-center">
        <a
          href={getGoogleCeremonyUrl(calendarStrings)}
          target="_blank"
        ...
        <a
          href={getOutlookUrl(calendarStrings)}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center justify-center gap-2 rounded-full border border-stone-300 bg-white px-5 py-2.5 text-xs font-medium tracking-wide text-stone-700 transition-colors hover:border-stone-400 hover:bg-stone-50"
        >
